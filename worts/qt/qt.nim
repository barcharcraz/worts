import nworts, os, strformat, strutils
...
var lbin = bin
lbin.url = fmt"""http://download.qt.io/official_releases/qt/${lbin.ver[0..lbin.ver.rfind(".")-1]}/{lbin.ver}/qt-opensource-linux-x64-{lbin.ver}.run.meta4"""
...
lbin.build = proc(pkg: PkgInstall) =
  shell(fmt"chmod +x {pkg.download_dir}/qt-opensource-linux-x64-{lbin.ver}.run")
  shell(fmt"""{pkg.download_dir}/qt-opensource-linux-x64-{lbin.ver}.run --script {pkg.src_dir}/installscript.qs --no-force-installations""")


var wbin = bin
wbin.url = fmt"""http://download.qt.io/official_releases/qt/${wbin.ver[0..wbin.ver.rfind(".")-1]}/{wbin.ver}/qt-opensource-windows-x86-{wbin.ver}.exe.meta4"""
...
wbin.build = proc(pkg: PkgInstall) =
  shell(fmt"""{pkg.download_dir}/qt-opensource-windows-x86-{wbin.ver}.exe --script {pkg.src_dir}/installscript.qs --no-force-installations""")
